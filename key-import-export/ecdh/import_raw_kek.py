...
def get_or_create_sender_credentials():
    ...
    subject = issuer = x509.Name([
        ...
        x509.NameAttribute(NameOID.COMMON_NAME, u"Sender CA"),
    ])
...
def main():
    parser = argparse.ArgumentParser(description="Import an AES-256 KEK into AWS Payment Cryptography using ECDH")
    parser.add_argument("--region", required=True, help="AWS Region")
    parser.add_argument("--profile", required=True, help="AWS Profile")
    parser.add_argument("--kek", required=True, help="Cleartext AES-256 KEK to import (Hex)")
    ...
    cert_response = client.get_public_key_certificate(
        KeyIdentifier=receiver_key_arn
    )
    ...
    AES_KEK_256_HEX = args.kek.upper()
    print(f"Using KEK to import: {AES_KEK_256_HEX}")
    ...
    # We are importing an AES-256 KEK (Key Usage 'K0', Algorithm 'A', Mode 'B' for Both)
    # Using Version 'D' (AES Key Derivation Binding Method)
    # Exportability 'E' (Exportable)
    tr31_header = "D0000K0AB00E0000"

    key_to_wrap = bytes.fromhex(AES_KEK_256_HEX)

    tr31_key_block = psec.tr31.wrap(
        kbpk=aes_wrapping_key,
        header=tr31_header,
        key=key_to_wrap
    )
